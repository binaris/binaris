...
const removeHandler = async function removeHandler(options) {
  const meta = await gatherMeta(options, false);
  await remove(meta.name);
  logger.info(
...
const listHandler = async function listHandler(options) {
  ...
  if (options.json) {
    ...
    const listData = Object.keys(listedFuncs).reduce((aggr, key) => {
      aggr[key] = `[${listedFuncs[key].tags.latest.modifiedAt}]`;
      return aggr;
    }, {});
    if (listData.length > 0) {
      logger.info(columnify(listData, { columns: ['Function', 'Last Deployed'] }));
    }
  }
};
