...
const gatherMeta = async function gatherMeta(name, funcPath, forcePath = true) {
  const metaObj = {
    path: path.resolve(funcPath || process.cwd()),
  };
  ...
  if (forcePath) {
    ...
    metaObj.printPath = funcPath ? ` -p ${metaObj.path}` : '';
  }
  metaObj.printName = name ? ` ${metaObj.name}` : '';
  return metaObj;
};

async function settleAndPrint(promises, names) {
  let hasFailures = false;
  await settle(promises).then((results) => {
    results.forEach((result, idx) => {
      const header = `Function ${names[idx]}:\n  `;
      if (result.isFulfilled()) {
        logger.info(`${header}${result.value()}`);
      } else {
        hasFailures = true;
        logger.error(`${header}${result.reason().message}`);
      }
    });
  });
...
const createHandler = async function createHandler(options) {
  ...
  const funcName = await create(options.function, funcPath,
    options.runtime, options.executionModel);
...
const deployHandler = async function deployHandler(options) {
  const allFunctions = [options.function, ...options.otherFunctions];
  const accountId = await getAccountId(undefined);

  async function simpleDeploy(name, path) {
    const meta = await gatherMeta(name, path);
  ...
  if (allFunctions.length === 1) {
    const output = await simpleDeploy(options.function, options.path);
    logger.info(output);
  } else {
    const deployPromises = allFunctions.map(async functionName => {
      const output = await simpleDeploy(functionName, options.path);
      return output;
    });
    await settleAndPrint(deployPromises, allFunctions);
  }
};
...
const removeHandler = async function removeHandler(options) {
  const removalOutput = 'Removed function';
  const allFunctions = [options.function, ...options.otherFunctions];

  if (allFunctions.length === 1) {
    await remove(options.function);
    logger.info(`${removalOutput} ${options.function}`);
  } else {
    const removePromises = allFunctions.map(async functionName => {
      await remove(functionName);
      return `${removalOutput} ${functionName}`;
    });
    await settleAndPrint(removePromises, allFunctions);
  }
};
