...
/**
 * Prints some given advice if it is defined.
 *
 * @param {string} advice - helpful advice to print to the user
 */
const function printAdvice(advice) {
  if (advice) {
    logger.warn(advice);
  }
}
...
const deployHandler = async function deployHandler(options) {
  ...
  const { response, advice } = await deploy(meta.name, meta.path, meta.conf);
  logger.info(
`Deployed function to ${deployResponse.response}
