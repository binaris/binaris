...
async function triggerUpsertHandler(options) {
  // options = {"_":["triggers","remove","triggerName"],"$0":"bn",...otherOptions}
  // We have to use the position number here because of lack of good support
  // for nested commands with positional arguments
  // consider replacing with `nested-yargs` npm package
  const triggerName = options._[2];
  const { sourceUri, target, errorHandler } = options;
  ...
  if (errorHandler) {
    triggerOptions.errorHandler = {
      funcName: errorHandler,
    };
  }
  const { status } = await triggers.upsert(triggerName, triggerOptions);
  logger.info(status);
}

async function triggerRemoveHandler(options) {
  // options = {"_":["triggers","remove","triggerName"],"$0":"bn"}
  // We have to use the position number here because of lack of good support
  // for nested commands with positional arguments
  // consider replacing with `nested-yargs` npm package
  const triggerName = options._[2];
  const { status } = await triggers.remove(triggerName);
  logger.info(status);
}
...
function formatTriggerSource(source) {
  ...
  const { proto, tls, host, port, db, streamName, password } = source;
  const dbPart = db ? `/${db}` : '';
  const portPart = port !== DEFAULT_REDIS_PORT ? `:${port}` : '';
  const passwordPart = password ? `:${password}@` : '';
  const tlsPart = tls ? '+tls' : '';
  return `${proto}${tlsPart}://${passwordPart}${host}${portPart}${dbPart}#${streamName}`;
}

async function triggerListHandler(options) {
  const result = await triggers.list();
  const rawData = result.list.map(({ name, config, updatedAt }) => ({
    name,
    target: config.target.funcName,
    source: formatTriggerSource(config.source),
    errorHandler: config.errorHandler ? config.errorHandler.funcName : undefined,
    lastUpdate: updatedAt,
  }));
  if (options.json) {
    logger.info(JSON.stringify(rawData));
  } else if (rawData.length === 0) {
    logger.info('No triggers defined in the system.');
  } else {
    const minWidth = 20;
    logger.info(columnify(rawData, {
      columns: ['name', 'source', 'target', 'lastUpdate'],
      config: {
        name: {
          headingTransform: () => 'TRIGGER',
          minWidth,
        },
        source: {
          minWidth,
...
module.exports = {
  ...
  triggerListHandler: exceptionWrapper(triggerListHandler),
  triggerUpsertHandler: exceptionWrapper(triggerUpsertHandler),
  triggerRemoveHandler: exceptionWrapper(triggerRemoveHandler),
};
