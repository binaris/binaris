...
const exceptionWrapper = function tryCatchWrapper(funcToWrap) {
  ...
  return async function wrapped(options) {
    try {
      ...
      if (process.env.BINARIS_LOG_LEVEL === 'info' || !process.env.BINARIS_LOG_LEVEL) {
        message = `${message}\nTip: You can export BINARIS_LOG_LEVEL=[silly|debug|verbose|info|warn|error] to view debug logs`;
      }
