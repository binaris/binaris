 ...
 */
const showHandler = async function showHandler(options) {
  if (options.account_id) logger.info(await getAccountId());
  if (options.api_key) logger.info(await getAPIKey());
}
