...
const exceptionWrapper = function tryCatchWrapper(funcToWrap) {
  ...
  return async function wrapped(argData) {
    try {
      ...
      if (err.errno > -112 && err.errno < -1) {
        const rawError = err.message.replace(/[\s\S]*:/,'');
        const cleanedUp = rawError.charAt(1).toUpperCase() + rawError.slice(2);
        logger.error(cleanedUp);
      } else {
