...
const perfHandler = async function perfHandler(options) {
  ...
  const upToString = options.maxSeconds ? ` up to ${options.maxSeconds} second${maybeS(options.maxSeconds)}` : '';
  logger.info(`Running performance test on function ${options.function}.
Executing ${options.maxRequests} invocations with ${options.concurrency} "thread${maybeS(options.concurrency)}"${upToString}.
Stand by for results...
`);
  const report = await perf(options.function, options.maxRequests,
    options.concurrency, options.data, options.maxSeconds);
