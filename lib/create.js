...
const create = async function create(functionName, functionPath,
  ...
  if (experimentalRuntimes.includes(runtime) && (config.entrypoint !== undefined || config.file !== undefined)) {
    throw new Error(`Configurable "entrypoint" and "file" are not supported for ${runtime}`);
  }
  ...
  if (!experimentalRuntimes.includes(runtime)) {
